...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, layers, lb, ub):
        ...
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, amsgrad=False)
    ...
    def net_f(self, x, t):
        ...
        f_rhs = tf.math.exp(-t)*tf.math.sin(2*np.pi*x)*(4*np.pi*np.pi-1) ##########
        f = u_t - u_xx - f_rhs  ########
        return f
    ...
    def loss_glb(self,x_0,t_0,u_0,x_l,t_l,x_u,t_u,x_f,t_f):
        u_ub = 0.0  # MSEb
        u_x_lb = 2 * np.pi * np.exp(-tb)  # MSEb

        u0_pred, _ = self.net_u(x_0, t_0)
        u_lb_pred, _ = self.net_u(x_l, t_l)
        _, u_x_ub_pred = self.net_u(x_u, t_u)
        loss_prd = tf.reduce_mean(tf.square(u_0 - u0_pred)) + \
                   tf.reduce_mean(tf.square(u_lb_pred - u_ub)) + \
                   tf.reduce_mean(tf.square(u_x_lb - u_x_ub_pred))
    ...
    def train(self, nIter):
        ...
        results = tfp.optimizer.lbfgs_minimize(
            value_and_gradients_function=func, initial_position=init_params, max_iterations=50000,
...
if __name__ == "__main__":
    ...
    # Domain bounds
    lb = np.array([0.0, 0.0]) #x,t
    ub = np.array([1.0, 0.5]) #x,t #####################
    ...
    data = scipy.io.loadmat('Data/CNdata.mat') ##########newdata_01t.mat
